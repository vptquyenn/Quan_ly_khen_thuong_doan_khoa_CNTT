...
if(isset($_POST['submit'])){
    ...
    $objReader->setLoadSheetsOnly('kt');

    $objExcel = $objReader->load($file);
    $sheetData = $objExcel->getActiveSheet()->toArray('null','null','null','null','null','null','null','null','null');
    ...
    for($row = 2; $row<=$highestRow; $row++){
        ...
        $noidung_hoatdong = $sheetData[$row]['E'];
        $hoc_ki = $sheetData[$row]['F'];
        $diemrl = $sheetData[$row]['G'];
        $xeploai = $sheetData[$row]['H'];
        $date_entered = $sheetData[$row]['I'];

        $sql = "INSERT INTO list_student(mssv,ho_ten,lop,khoa,noidung_hoatdong,hoc_ki,diemrl,xeploai,date_entered) VALUES ('$mssv','$ho_ten','$lop','$khoa','$noidung_hoatdong','$hoc_ki','$diemrl','$xeploai','$date_entered')";
        //print_r($sql);
...
<body onload="time()">
    ...
    <div class="container-fluid al">
        ...
        <form name="frm" id="frm" action="" enctype="multipart/form-data" method="post" class="col-md-6 col-md-offset-3" style="margin-left: 350px">
            <h2 class='text-center text-primary m-0 ps-5'>Import Danh Sách Khen Thưởng Sinh Viên</h2>

            <div class="form-group" style="margin-bottom: 20px; margin-top: 40px;">
                <label for="">Flie Khen Thưởng Sinh Viên</label>
